...
- middle (random location, after some node based on value)
* Deleting a node in a linked list - delete a node in a given linked list
- first node(before the head),
- last node(after the tail) and
- middle (random location, after some node based on value)
